...
bounded, and serialized writes use the exact same-directory
`.<destination-filename>.tmp` path followed by atomic replacement.
Cross-process activation and provider ownership use non-blocking
...
temporaries use that exact `.<destination-filename>.tmp` transformation, are
removed on every handled path, and are safely reclaimed by the next owner after
a crash, leaving at most one per fixed destination. They never become
candidates, although they still consume the
